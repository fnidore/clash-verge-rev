...
fn uninstall_service() -> Result<()> {
    ...
    if linux_running_as_root() {
        let cmd = StdCommand::new(&uninstall_path);
...
pub async fn is_service_available_cached() -> bool {
    ...
    let path_exists = is_service_ipc_path_exists().await;
    if !path_exists {
...
pub async fn is_service_available() -> Result<()> {
    // 先检查 IPC 路径是否存在（快速检查）
    if !is_service_ipc_path_exists().await {
